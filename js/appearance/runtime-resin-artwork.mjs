...
export function runtimeResinSettings(payload) {
  try {
    if (!payload || payload.v < 3 || !Array.isArray(payload.r)) {
      return {
    ...
    console.error('Failed to read runtime resin settings:', error);
    return {
      clearEnabled: false, opacity: 1, frost: 0, tintColor: '#ffffff',
      interiorEnabled: false, type: 'none', primary: '#ffffff', secondary: '#ffffff', density: 0, intensity: 0,
    };
...
export function runtimeResinDefs(resin) {
  try {
    const defs = [];
    ...
    console.error('Failed to build runtime resin defs:', error);
    return '';
  }
}

export function runtimeResinArtwork(resin, payload) {
  try {
    const parts = [];
    if (resin.clearEnabled) {
    ...
    console.error('Failed to build runtime resin artwork:', error);
    return '';
  }
}
