...
const DEFAULT_DICE_COLOR = SYSTEM_DEFAULT_DICE_SET.appearance.diceSet.defaultStyle.bodyColor;
const DEFAULT_TRAY_COLOR = SYSTEM_DEFAULT_DICE_SET.appearance.tray.color;
...
export function buildLivePhysicsConfig(runtime, legacyThemeColor = DEFAULT_DICE_COLOR) {
  ...
  return {
    mode: 'custom', themeColor: safeHex(runtime.defaultThemeColor, DEFAULT_DICE_COLOR), runtimeThemes,
    diceBoxOptions: { externalThemes, offscreen: false },
  };
}
function defaultTrayVisual() {
  return {
    active: true,
    background: `radial-gradient(circle at 50% 35%, ${DEFAULT_TRAY_COLOR}, #020617 82%)`,
    imageUrl: null,
    shadow: DEFAULT_TRAY_SHADOW,
  };
}
export function buildLiveTrayVisual(runtime) {
  if (runtime?.mode !== 'custom') return defaultTrayVisual();
