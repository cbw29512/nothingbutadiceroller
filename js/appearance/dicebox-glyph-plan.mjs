...
function centeredCommand(dieType, logicalResult, die, region) {
  ...
  return {
    ...
    color: faceColor(face, die.style), fontId: face?.fontId || null, region: structuredClone(region),
  };
}
function d4Command(logicalResult, die, anchor) {
  ...
  return {
    ...
    color: faceColor(face, die.style), fontId: face?.fontId || null, marks: structuredClone(anchor.marks),
  };
}
