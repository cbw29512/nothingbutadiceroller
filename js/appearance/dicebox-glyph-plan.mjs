...
function faceColor(face, style) {
  return face?.color || style.faceColor;
}
...
function centeredCommand(dieType, logicalResult, die, region) {
  ...
  return {
    ...
    text: faceText(face, dieType, logicalResult),
    color: faceColor(face, die.style),
...
function d4Command(logicalResult, die, anchor) {
  ...
  return {
    dieType: 'd4',
    logicalResult,
    strategy: 'tetrahedral-vertex-repeat',
    text: faceText(face, 'd4', logicalResult),
    color: faceColor(face, die.style),
    fontId: face?.fontId || null,
    marks: structuredClone(anchor.marks),
  };
}
...
export function buildDiceBoxGlyphPlan(renderPlan, modelData) {
  try {
    const commands = [];
    const d4Anchors = extractD4VertexAnchors(modelData);
    for (const type of Object.keys(CANONICAL_DICE)) {
      ...
      if (type === 'd4') {
        for (const logicalResult of getCanonicalFaceResults(type)) {
          commands.push(d4Command(logicalResult, die, d4Anchors[String(logicalResult)]));
        }
        continue;
      }
      const regions = extractDiceBoxFaceRegions(modelData, type);
      for (const logicalResult of getCanonicalFaceResults(type)) {
        commands.push(centeredCommand(type, logicalResult, die, regions[String(logicalResult)]));
      }
    }
    return { atlasVersion: 1, sourceSetId: renderPlan.sourceSetId, commands };
  } catch (error) {
    console.error('Failed to build DiceBox glyph plan:', error);
    throw error;
  }
}
