...
function emit(event, user) {
  listeners.get(event)?.forEach((callback) => {
    try { callback(user); }
    catch (error) { console.error(`Auth ${event} listener failed:`, error); }
  });
}
...
export function getIdentity() { return identityFacade; }

export function setAccountUser(user, { initial = false } = {}) {
  const previous = currentUser;
  currentUser = user || null;
  if (initial) { emit('init', currentUser); return; }
...
function ensureAuthSubscription() {
  ...
  onAuthChange((event, user) => {
    try {
      if (event === 'logout') { setAccountUser(null); return; }
      if (user) setAccountUser(user);
    } catch (error) { console.error('Netlify Identity auth-change handler failed:', error); }
  });
}
...
export async function logoutAccount() { ensureAuthSubscription(); await logout(); setAccountUser(null); }
...
export async function acceptInviteAccount(token, password) {
  ...
  const user = await acceptInvite(token, password); setAccountUser(user); return user;
}
export async function completeRecoveryAccount(password) {
  ensureAuthSubscription();
  if (!password) throw new Error('Enter a new password.');
  const user = await updateUser({ password }); setAccountUser(user); return user;
}
