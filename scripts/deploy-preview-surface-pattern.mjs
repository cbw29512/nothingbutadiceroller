...
async function setControl(client, selector, value, eventName = 'input') {
  try {
    ...
    console.error('Failed to set live surface-pattern control:', selector, error);
    throw error;
  }
}
...
async function configureDesktop(client) {
  ...
  await client.evaluate(`(() => {
    ...
    name.value = 'Live Marble Pattern';
...
async function verifyDesktopRoll(client) {
  ...
  assert.ok(roll.total >= 1 && roll.total <= 20, `Live marble d20 must remain 1-20; received ${roll.total}.`);
  assert.ok(roll.diffuse.length >= 1, 'Live marble d20 must request a generated diffuse texture.');
  let found = false;
  for (const url of roll.diffuse) {
    const response = await fetch(url);
    if (!response.ok) continue;
    const text = await response.text();
    if (text.includes('patternMarble') && text.toLowerCase().includes('#7c3aed')) found = true;
  }
  assert.equal(found, true, 'Live physical d20 texture must contain the selected marble pattern artwork.');
}
...
async function verifyMobileScope(client) {
  ...
  assert.equal(state.d20, 'none');
  assert.equal(state.overflow, false);
  assert.ok(state.selectHeight >= 40, 'Live mobile surface-pattern picker must remain comfortably tappable.');
}
...
async function run() {
  ...
  try {
    ...
    console.log('Live surface pattern passed: marble Save/Use, hosted generated texture, physical d20 mechanics, and mobile selected-d6 speckle are protected.');
  } finally {
    if (browser) await browser.close().catch((error) => console.warn('Browser cleanup failed:', error.message));
  }
}
run().catch((error) => { console.error('Live surface pattern failed:', error); process.exitCode = 1; });
