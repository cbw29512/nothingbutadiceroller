...
set = replaceVisualFace(set, 'd20', 20, { kind: 'text', value: '☠', color: '#a855f7', fontId: 'fantasy', scale: 1.2, position: 'top-right' });
...
assert.equal(plan.dice.d20.faces['20'].position, 'top-right', 'Approved bounded glyph position must survive the visual render-plan projection.');
assert.equal(plan.dice.d20.faces['1'].scale, 1, 'Legacy/custom faces without scale must resolve to the 100% visual default.');
assert.equal(plan.dice.d20.faces['1'].position, 'center', 'Legacy/custom faces without position must resolve to center.');
...
console.log('Appearance render plan passed: bounded face typography/scale/position and per-die visuals survive projection; roll mechanics and physics configuration remain excluded.');
