...
const HOSTED_ROUTE_TOKEN_CAP = 1800;
...
assert.ok(runtime.token.length < HOSTED_ROUTE_TOKEN_CAP, `Edge-inlay runtime token must remain below ${HOSTED_ROUTE_TOKEN_CAP} characters; received ${runtime.token.length}.`);
assert.equal(encodeRuntimeThemePayload(payload), runtime.token, 'Compact v6 edge-inlay tokens must be stable after decode/re-encode.');
...
for (const dieType of Object.keys(CANONICAL_DICE)) {
  ...
  assert.ok(realRuntime.token.length < HOSTED_ROUTE_TOKEN_CAP, `${dieType} inlay token must remain below ${HOSTED_ROUTE_TOKEN_CAP} characters; received ${realRuntime.token.length}.`);
  assert.equal(encodeRuntimeThemePayload(realPayload), realRuntime.token, `${dieType} compact runtime token must be stable after decode/re-encode.`);
  assert.ok(buildRuntimeThemeSvg(realPayload).includes('id="edgeInlay"'));
}

const legacyV6Payload = structuredClone(payload);
const legacyV6Token = Buffer.from(JSON.stringify(legacyV6Payload), 'utf8').toString('base64url');
assert.deepEqual(decodeRuntimeThemePayload(legacyV6Token), legacyV6Payload, 'Already-open object-form v6 tokens must remain readable after compact encoding ships.');
...
console.log('Edge inlay contract passed: four bounded styles, real-model physical edge segments across UV seams for all seven standard dice, compact hosted-route-safe v6 tokens, set/per-die inheritance, legacy compatibility, mechanics isolation, and fail-closed validation are protected.');
