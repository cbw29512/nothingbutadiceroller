...
for (const type of getSupportedFaceEditorDice()) {
  ...
  layout.forEach((face) => {
    assert.ok(face.x >= 6 && face.x <= 94, `${type} face ${face.logicalFace} must stay within horizontal editor bounds.`);
    assert.ok(face.y >= 6 && face.y <= 94, `${type} face ${face.logicalFace} must stay within vertical editor bounds.`);
  });
}
...
custom = replaceVisualFace(custom, 'd20', 1, { kind: 'text', value: 'A', color: '#ffffff', fontId: 'fantasy' });
...
custom = replaceVisualFace(custom, 'd100', 0, { kind: 'text', value: '100', color: '#ffffff' });
assert.equal(validateDiceSet(custom).ok, true, 'Numbers, one character, and one symbol are visual-only and valid.');
...
assert.equal(getVisualFace(custom, 'd100', 0).value, '100');
...
assert.throws(() => replaceVisualFace(custom, 'd20', 20, { kind: 'text', value: 'CRIT' }));
assert.throws(() => replaceVisualFace(custom, 'd20', 20, { kind: 'text', value: 'FIRE' }));
assert.throws(() => replaceVisualFace(custom, 'd20', 20, { kind: 'text', value: 'AB' }));
assert.throws(() => replaceVisualFace(custom, 'd20', 20, { kind: 'text', value: '🔥🔥' }));
assert.throws(() => replaceVisualFace(custom, 'd100', 0, { kind: 'text', value: 'BOOM' }));
