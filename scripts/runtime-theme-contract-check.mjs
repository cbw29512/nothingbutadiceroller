...
for (const dieType of Object.keys(CANONICAL_DICE)) {
  const diePayload = {
    ...
    g: [false, '#ffffff', 0], f: standardFinish, p: standardPattern,
  };
...
const legacyV4Payload = { ...payload, v: 4 };
delete legacyV4Payload.p;
assert.equal(validateRuntimeThemePayload(legacyV4Payload).ok, true, 'Version 4 finish tokens must remain readable after patterns ship.');
assert.equal(buildRuntimeThemeSvg(legacyV4Payload).includes('patternMarble'), false);
...
console.log('Runtime theme contract passed: short labels, path-safe tokens, glow/finish/pattern textures, v1-v4 token compatibility, canonical mesh reuse, safe SVG, and mechanics-field rejection are protected.');
