...
const boundariesFor = (dieType) => getCanonicalFaceResults(dieType).map((_, index) => {
  ...
  return [offset, 20, offset + 18, 20, offset + 18, 20, offset + 9, 38, offset + 9, 38, offset, 20];
});
const payload = {
  v: RUNTIME_THEME_VERSION, d: 'd20', s: 1024,
  o: [['<', '#a855f7', 'fantasy', 512, 512, 64]],
  g: [true, '#00ffcc', 0.75], f: standardFinish, p: standardPattern, i: standardInlay,
};
const token = encodeRuntimeThemePayload(payload);
assert.match(token, /^[A-Za-z0-9_-]+$/); assert.deepEqual(decodeRuntimeThemePayload(token), payload);
assert.equal(encodeRuntimeThemePayload(decodeRuntimeThemePayload(token)), token); assert.ok(token.length < 6000);
const config = buildRuntimeThemeConfig(payload);
assert.deepEqual(config.diceAvailable, ['d20']); assert.equal(config.systemName, buildRuntimeThemeIdentity('d20', token));
assert.equal(config.material.type, 'color'); assert.equal(config.material.diffuseTexture.light, 'diffuse.svg');
assert.equal('meshFile' in config, false); assert.equal('bumpTexture' in config.material, false);
for (const dieType of Object.keys(CANONICAL_DICE)) {
  const diePayload = { v: RUNTIME_THEME_VERSION, d: dieType, s: 1024, o: operation('1'), g: [false, '#ffffff', 0], f: standardFinish, p: standardPattern, i: standardInlay };
  const dieToken = encodeRuntimeThemePayload(diePayload); const dieConfig = buildRuntimeThemeConfig(diePayload);
  assert.equal(dieConfig.systemName, buildRuntimeThemeIdentity(dieType, dieToken)); assert.deepEqual(dieConfig.diceAvailable, [dieType]);
}
...
assert.ok(svg.includes('filter="url(#numberGlow)"'), 'Runtime face halo must use the glow filter when enabled.');
assert.ok(svg.includes('<feMorphology'), 'Runtime glow must expand glyph alpha before blurring so the baked halo remains visibly substantial.');
assert.ok(svg.includes('data-number-glow="halo"'), 'Runtime glow must render a dedicated colored halo layer behind the face glyph.');
assert.ok(svg.includes('data-number-glow="face"'), 'Runtime glow must preserve a separate crisp face-glyph layer above the halo.');
const monoSvg = buildRuntimeThemeSvg({ ...payload, o: [['MONO', '#ffffff', 'mono', 512, 512, 64]] });
assert.ok(monoSvg.includes('font-family="&quot;Courier New&quot;, monospace"'), 'Quoted Mono / Arcane Tech font stacks must be XML-escaped in runtime SVG.');
assert.equal(monoSvg.includes('font-family=""Courier New"'), false, 'Runtime SVG must never emit an unescaped quoted font-family attribute.');
...
assert.equal(noGlowSvg.includes('data-number-glow="halo"'), false, 'Disabled number glow must not emit a halo layer.');
const finishSvg = buildRuntimeThemeSvg(changedFinish);
assert.ok(finishSvg.includes('surfaceMetallic'), 'Metallic finish must render into the generated diffuse texture.'); assert.ok(finishSvg.toLowerCase().includes('#f59e0b'));
const patternSvg = buildRuntimeThemeSvg(changedPattern);
assert.ok(patternSvg.includes('patternMarble'), 'Marble pattern must render into the generated diffuse texture.'); assert.ok(patternSvg.toLowerCase().includes('#7c3aed'));
...
assert.ok(inlaySvg.toLowerCase().includes('#22d3ee')); assert.match(inlaySvg, /stroke-dasharray=/);
...
const legacyV4Payload = { v: 4, d: 'd20', s: 1024, o: operation('20'), g: [false, '#ffffff', 0], f: ['metallic', '#f59e0b', 0.8] };
...
const legacyV5Payload = { v: 5, d: 'd20', s: 1024, o: operation('20'), g: [false, '#ffffff', 0], f: standardFinish, p: ['marble', '#f8fafc', '#7c3aed', 0.7, 0.6] };
...
assert.ok(functionSource.includes("path: '/api/dice-theme/:token/:asset'")); assert.ok(functionSource.includes("asset === 'theme.config.json'"));
assert.ok(functionSource.includes("asset === 'diffuse.svg'")); assert.ok(functionSource.includes("'X-Content-Type-Options': 'nosniff'"));
console.log('Runtime theme contract passed: visible baked glow layers, XML-safe fonts, short labels, path-safe v6 tokens, finish/pattern/segmented-inlay textures, v1-v5 compatibility, canonical mesh reuse, safe SVG, and mechanics-field rejection are protected.');