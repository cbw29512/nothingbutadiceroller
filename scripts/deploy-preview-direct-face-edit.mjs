...
async function run() {
  ...
  try {
    ...
    assert.match(handoff.snapshot, /CRIT/); assert.match(handoff.snapshot, /fantasy/); assert.match(handoff.snapshot, /"scale":1\.2/); assert.match(handoff.snapshot, /"position":"top-right"/);
    ...
    assert.ok(roll.total >= 1 && roll.total <= 20, `Live positioned/scaled typography d20 must remain 1-20; received ${roll.total}.`);
    ...
    assert.equal(customizedTexture, true, 'Live generated d20 texture must contain bounded positioned Fantasy CRIT artwork.');
    ...
    assert.equal(mobileState.overflow, false, 'Live typography/scale/position controls must not create horizontal overflow.');

    console.log('Live Studio face-art handoff passed: immutable Default auto-copy, Fantasy CRIT at 120% Top Right, Save/Use persistence, bounded generated texture, mobile usability, and canonical d20 mechanics all persist.');
  } finally {
