...
async function run() {
  ...
  try {
    ...
    const selected = await client.evaluate(`(() => ({
      ...
      editable: !document.querySelector('#set-name')?.disabled && !document.querySelector('#face-value')?.disabled && !document.querySelector('#custom-face-color')?.disabled && !document.querySelector('#face-font')?.disabled && !document.querySelector('#face-scale')?.disabled && !document.querySelector('#apply-face')?.disabled,
    ...
    assert.equal(selected.font, 'default'); assert.equal(selected.scale, '100'); assert.equal(selected.editable, true); assert.equal(selected.focused, true);
    ...
    const preview = await client.evaluate(`(() => ({
      ...
      scaleOutput: document.querySelector('#face-scale-output')?.textContent || '', previewFont: document.querySelector('.studio-preview-die[data-die="d20"] span')?.style.fontFamily || '',
      previewSize: document.querySelector('.studio-preview-die[data-die="d20"] span')?.style.fontSize || '', mapFont: document.querySelector('#face-map .face-node.active')?.style.fontFamily || '',
      mapSize: document.querySelector('#face-map .face-node.active')?.style.fontSize || '', useLabel: document.querySelector('#use-set')?.textContent || '',
    }))()`);
    assert.equal(preview.mode, 'custom'); assert.equal(preview.font, 'fantasy'); assert.equal(preview.scale, '120'); assert.equal(preview.scaleOutput, '120%');
    ...
    assert.ok(parseFloat(preview.mapSize) > 0.72); assert.match(preview.useLabel, /Use This Set.*Back to Roller/i);
    ...
    assert.match(handoff.snapshot, /CRIT/); assert.match(handoff.snapshot, /fantasy/); assert.match(handoff.snapshot, /"scale":1\.2/);
    ...
    assert.ok(roll.total >= 1 && roll.total <= 20, `Live scaled typography d20 must remain 1-20; received ${roll.total}.`);
    assert.ok(roll.diffuse.length >= 1);
    let scaledTexture = false;
    for (const url of roll.diffuse) {
      const response = await fetch(url); if (!response.ok) continue;
      const text = await response.text();
      const crit = text.match(/<text[^>]*font-family="Georgia, serif"[^>]*font-size="([0-9.]+)"[^>]*>CRIT<\/text>/);
      if (crit && Number(crit[1]) >= 6 && Number(crit[1]) <= 200) scaledTexture = true;
    }
    assert.equal(scaledTexture, true, 'Live generated d20 texture must contain bounded Fantasy CRIT artwork after 120% scaling.');
    ...
    assert.equal(mobileState.scale, '120'); assert.equal(mobileState.scaleOutput, '120%');
    assert.equal(mobileState.overflow, false, 'Live typography/scale controls must not create horizontal overflow.');

    console.log('Live Studio face-art handoff passed: immutable Default auto-copy, Fantasy CRIT at 120%, Save/Use persistence, bounded generated texture, mobile usability, and canonical d20 mechanics all persist.');
  } finally {
