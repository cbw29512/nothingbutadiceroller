...
try {
  ...
  [
    ...
    'extractTrayImageDataUrl', 'prepareCloudDiceSet(rawSet, user.id)', 'assertLockedUpdateAllowed(existing.set, set)',
  ...
  [
    ...
    'openDiceSetStore(context)', "request.headers.get('If-Match')", 'conditionalRecordWrite(store, key, tombstone, expectedVersion)',
    'await store.delete(key)', 'verifyRequestOrigin(request)', "Request origin is not allowed.",
  ...
  console.log('Dice-set API contract passed: optimistic concurrency, fail-closed Community revisions, safe stale-delete handling, anonymous owner metadata, storage isolation, image lifecycle, and recoverable Studio conflicts are enforced.');
} catch (error) {
