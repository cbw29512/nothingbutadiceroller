...
set = replaceVisualFace(set, 'd20', 20, { kind: 'icon', value: 'skull', color: '#a855f7', scale: 1.2, position: 'top-right' });
set = replaceVisualFace(set, 'd4', 1, { kind: 'text', value: 'ᚱ', color: '#ffffff', scale: 0.6, position: 'bottom-left' });
...
assert.equal(command('d20', 20).text, '☠'); assert.equal(command('d20', 20).color, '#a855f7'); assert.equal(command('d20', 20).scale, 1.2); assert.equal(command('d20', 20).position, 'top-right');
assert.equal(command('d20', 19).text, '19'); assert.equal(command('d20', 19).scale, 1, 'Untouched faces must compile at the 100% default.'); assert.equal(command('d20', 19).position, 'center', 'Untouched faces must compile at center.');
assert.equal(command('d10', 10).text, '0');
assert.equal(command('d100', 0).text, '00'); assert.equal(command('d100', 90).text, 'MAX'); assert.equal(command('d100', 90).scale, 1); assert.equal(command('d100', 90).position, 'center');
assert.equal(command('d100', 90).logicalResult, 90, 'The word MAX must remain visual artwork over logical result 90.');
assert.equal(command('d4', 1).strategy, 'tetrahedral-vertex-repeat'); assert.equal(command('d4', 1).text, 'ᚱ'); assert.equal(command('d4', 1).scale, 0.6); assert.equal(command('d4', 1).position, 'bottom-left'); assert.equal(command('d4', 1).marks.length, 3);
assert.equal(glyphs.commands.length, 70); assert.equal('roll' in glyphs, false); assert.equal('result' in glyphs, false);
console.log('DiceBox glyph plan passed: canonical labels, short words/symbols, bounded per-face scale/position, percentile faces, and d4 markings stay visual-only.');
