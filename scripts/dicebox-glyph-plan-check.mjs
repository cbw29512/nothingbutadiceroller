...
assert.equal(command('d20', 20).text, '☠'); assert.equal(command('d20', 20).color, '#a855f7');
assert.equal(command('d20', 19).text, '19'); assert.equal(command('d10', 10).text, '0');
assert.equal(command('d100', 0).text, '00'); assert.equal(command('d100', 90).text, 'MAX');
assert.equal(command('d100', 90).logicalResult, 90, 'The word MAX must remain visual artwork over logical result 90.');
assert.equal(command('d4', 1).strategy, 'tetrahedral-vertex-repeat'); assert.equal(command('d4', 1).text, 'ᚱ'); assert.equal(command('d4', 1).marks.length, 3);
assert.equal(glyphs.commands.length, 70); assert.equal('roll' in glyphs, false); assert.equal('result' in glyphs, false);
console.log('DiceBox glyph plan passed: canonical labels, short words/symbols, percentile faces, and d4 markings stay visual-only.');
