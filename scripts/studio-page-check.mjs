...
try {
  const [
    ...
    faceMap, numberGlow, symbolPicker,
  ] = await Promise.all([
    ...
    read('js/appearance/studio-face-map.mjs'), read('js/appearance/number-glow.mjs'), read('js/appearance/face-symbol-picker.mjs'),
  ]);
  ...
  requireAll(render, [
    'getSupportedFaceEditorDice', 'renderFaceMap', 'safeTrayImageUrl', 'numberGlowTextShadow', 'buildSurfacePreviewBackground',
    'fillStudioSurfaceControls', 'fillStudioPatternControls', 'fillStudioInlayControls', 'die.dataset.surfacePattern',
    'die.dataset.edgeInlay', 'faceText.dataset.previewFace', 'renderPreview(set, selectedDie, faceNumber)',
  ], 'Studio render contract');
  requireAll(faceMap, [
    "import { numberGlowTextShadow } from './number-glow.mjs'", "glyph.dataset.numberGlow = style.glow?.enabled ? 'active' : 'off'",
    'glyph.style.textShadow = numberGlowTextShadow(style.glow)',
  ], 'face-map glow contract');
  requireAll(numberGlow, [
    'export function numberGlowTextShadow', '0 0 2px', 'const wide = Math.round(13 + (17 * intensity))',
  ], 'shared number-glow contract');
  requireAll(symbolPicker, [
    'apply it immediately to the selected face', "if (typeof applyFace !== 'function')", 'const applied = applyFace()',
    'Save the dice set to keep it.',
  ], 'one-tap symbol persistence contract');
  requireAll(bindings, [
    ...
    'applyFace: visualControls.applyFace',
  ...
  requireAll(visualControls, [
    'MAX_BROWSER_TRAY_IMAGE_BYTES', 'MAX_TRAY_IMAGE_BYTES', "kind: 'text'", "q('die-glow-enabled').addEventListener('change'",
    'export function applyStudioFaceAppearance', 'return { applyFace }',
  ...
  console.log('Studio page contract passed: shared glow rendering, one-tap symbol persistence, selected-face model preview, all buttons, and visual layers including UV edge inlays are handler-audited without weakening persistence, mechanics, cloud, tray, or navigation protections.');
} catch (error) {
