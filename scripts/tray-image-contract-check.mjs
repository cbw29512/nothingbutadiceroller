...
for (const url of ['javascript:alert(1)', 'https://evil.example/tray.png', 'data:image/svg+xml;base64,PHN2Zz4=', 'data:text/html;base64,PGgxPg==']) {
  assert.equal(validateTrayImage({ url }).ok, false, `Unsafe tray image must be rejected: ${url}`);
}
...
assert.match(saveSource, /users\/\$\{userId\}\/dice-sets\/\$\{setId\}_tray/);
...
console.log('Tray image contract passed: vetted formats only, 4 MB limit, capability delivery, lock protection, and Blob cleanup are enforced.');
