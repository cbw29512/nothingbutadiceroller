...
assert.match(imageSource, /openDiceSetStore\(\)/);
...
assert.match(deleteSource, /store\.delete\(publicRecordKey\(user\.id, setId\)\)/);
assert.match(deleteSource, /existing\.trayImageKey/);
assert.match(deleteSource, /store\.delete\(existing\.trayImageKey\)/);
console.log('Tray image contract passed: exact metadata, vetted formats, 4 MB limit, encoded strong-consistency storage keys, no-store capability delivery, public-projection cleanup, lock protection, and Blob cleanup are enforced.');
