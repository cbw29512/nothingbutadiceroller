...
const ownerDelete = deleteSource.indexOf('await store.delete(key);');
const imageCleanup = deleteSource.indexOf("bestEffortDelete(store, existing.trayImageKey, 'tray image')");
assert.ok(ownerDelete >= 0 && imageCleanup > ownerDelete, 'Owner record must be deleted before tray-image cleanup.');
console.log('Tray image contract passed: exact metadata, 512 KB browser / 4 MB cloud limits, versioned atomic image staging, preview-store isolation, authenticated opaque delivery, no-store responses, and owner-first cleanup are enforced.');
